...
static procmetrix_error_t swap_used_ratio(double *percent_swap_usage)
{
    ...
    if (ERROR_SUCCESS == PdhOpenQuery(NULL, 0, &query))
    {
        ...
        if (ERROR_SUCCESS == PdhAddEnglishCounterA(query, "\\Paging File(_Total)\\% Usage", 0, &counter))
        {
            PDH_FMT_COUNTERVALUE counter_value;
            if (ERROR_SUCCESS == PdhCollectQueryData(query) &&
                ERROR_SUCCESS == PdhGetFormattedCounterValue((PDH_HCOUNTER)counter, PDH_FMT_DOUBLE, 0, &counter_value))
            {
